...
void vehicle::rotate_the_car(){
	float get_angle = abs(getAngle({ this->old_dir.x, this->old_dir.y }));
	if (get_angle < abs(this->angle_of_rotate))
	{
		float x1, y1;
		float angle = angle_of_wheel;
		if (get_angle < this->angle_of_wheel)
			angle = this->angle_of_wheel - get_angle;
